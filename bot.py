...
def create_video(images: list, audio_path: str, script_data: dict) -> str:
    ...
    for img_path in images:
        ...
        # Ken Burns effect
        clip = clip.resize(lambda t: 1 + 0.05 * t / img_duration)
        clips.append(clip)
...
async def make_video(ctx, lang: str = 'ar', *, topic: str = None):
    ...
    try:
        ...
        # 4. تجميع الفيديو
        await msg.edit(content="🎞️ تجميع الفيديو... (هذا يأخذ دقيقة)")
        video_path = await asyncio.get_event_loop().run_in_executor(
            None, create_video, images, audio_path, script_data
        )
        
        # 5. إرسال الفيديو
        await msg.edit(content="📤 إرسال الفيديو...")
...
async def help_cmd(ctx):
    ...
    embed.add_field(
        name="!video ar [موضوع]",
        value="صنع فيديو بالعربي\nمثال: `!video ar فوائد شرب الماء`",
        inline=False
    )
    embed.add_field(
        name="!video en [topic]",
        value="Create English video\nExample: `!video en morning routine tips`",
        inline=False
    )
    await ctx.send(embed=embed)
